...
def get_fullhess(model, x, deg_of_freedom = DEGREES_OF_FREEDOM, device = torch.device("cpu")):
    x = x.to(device)
    H = vmap(torch.func.hessian(model), in_dims= 0)(x)
    return H
...
def plot_diff(y_real, y_pred, log_dir, name):
    fig = plt.figure(figsize=(12.8, 8), dpi=120)
    axes = fig.subplots(nrows=2)
    axes[0].plot(y_real, label='y traget')
    axes[1].plot(y_pred, label='y predicted')
...
def train_model(name, training_dataloader, test_dataloader, epochs = 10, deg_of_freedom = DEGREES_OF_FREEDOM, device = torch.device("cpu"), log = LOG_DIR):
    ...
    print(x)
    q_tt_pred = get_acc(model, x, deg_of_freedom, device)
...
def main():
    ...
    for i in range(start_idx, start_idx + n):
        ...
        x_train = 3 * torch.rand((2**12, 2 * deg_of_freedom))
        ...
        x_test = 3 * torch.rand((2**13, 2 * deg_of_freedom))
